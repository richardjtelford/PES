...
data.frame(
  forams = c(O2 = cor(chem$O2, rarefy(foram, 100)), TOC = cor(chem$TOC, rarefy(foram, 100))),
  macros = c(cor(chem$O2, rarefy(macro, 100)), cor(chem$TOC, rarefy(macro, 100)))
) %>% round(2)
...
data.frame(
  forams = c(O2 = cor(chem$O2, diversity(foram)), TOC = cor(chem$TOC, diversity(foram))),
  macros = c(cor(chem$O2, diversity(macro)), cor(chem$TOC, diversity(macro)))
) %>% round(2)
...
data.frame(
  forams = c(O2 = f02$CCA$tot.chi / f02$tot.chi * 100, TOC = ftoc$CCA$tot.chi / ftoc$tot.chi * 100),
  macros = c(m02$CCA$tot.chi / m02$tot.chi * 100, mtoc$CCA$tot.chi / mtoc$tot.chi * 100)
) %>% round(2)
...
tf.perf <- rbind(
  performance(crossval(waO2f))$crossval[1, , drop = FALSE],
  performance(crossval(waO2m))$crossval[1, , drop = FALSE],
  performance(crossval(watocf))$crossval[1, , drop = FALSE],
  performance(crossval(watocm))$crossval[1, , drop = FALSE]
)
