...
fall <- xtabs(Number.1 ~ Station_code + SpeciesForam, data = forams, drop = TRUE)
...
f63 <- xtabs(Number.1 ~ Station_code + SpeciesForam, data = forams[fsmall, ], drop = TRUE)
f125 <- xtabs(Number.1 ~ Station_code + SpeciesForam, data = forams[fbig, ], drop = TRUE)
...
f01 <- xtabs(Number.1 ~ Station_code + SpeciesForam, data = forams[shallow, ], drop = TRUE)
f12 <- xtabs(Number.1 ~ Station_code + SpeciesForam, data = forams[deep, ], drop = TRUE)
...
ffos <- xtabs(Number.1 ~ Station_code + SpeciesForam, data = forams[fos, ], drop = TRUE)
fnofos <- xtabs(Number.1 ~ Station_code + SpeciesForam, data = forams[nofos, ], drop = TRUE)

flargefos <- xtabs(Number.1 ~ Station_code + SpeciesForam, data = forams[fos & fbig, ], drop = TRUE)
...
sapply(fff, function(f)
  sum(rowSums(f) < 50))

fdiv <- lapply(fff, function(f)
  exp(div(f)))
...
ford <- lapply(fff, function(f)
  rda(sqrt(f / rowSums(f))))
m2 <- c(
  sapply(ford,function(f)sqrt(1 - procrustes(ford$all, f, sym = TRUE)$ss)),
  deadvsfos = sqrt(1 - procrustes(ford$fos, ford$dead, sym = TRUE)$ss))

of <- lapply(fff, function(f)
    rda(sqrt(f / rowSums(f)) ~ mO2, data = chem,  na.action = na.omit))
o2c <- c(sapply(of, function(f)
  f$CCA$tot.chi / f$tot.chi), deadvsfos = NA)
...
############
############ replicates
fallrep <- xtabs(Number.1 ~ paste(Station_code, Replicate, sep = ":") + SpeciesForam, data = forams[forams$DAT > 20050000, ], drop = TRUE)
...
x11()
plot(diversity(fmc), div(fmc))
x11()
plot(diversity(mfc), div(mfc))
...
x11(height = 6, 6, 11)
par(mfrow = c(2, 2), mar = c(3, 3, .5, .5), mgp = c(1.5, .5, 0), tcl = -.4)
...
##
#largefossilisable
ff125 <- xtabs(Number.1 ~ Station_code + SpeciesForam, data = forams[forams$DAT > 20050000 & fbig & fos, ], drop = TRUE)
ff125mc <- ffos[rownames(ffos) %in% sitesfmc, ]

x11(height = 3, 3, 11)
par(mfrow = c(1, 1), mar = c(3, 3, .5, .5), mgp = c(1.5, .5, 0))
...
x11()
plot(mod.dead, disp = c("sites", "cn"))
x11()
plot(mod.fos, disp = c("sites", "cn"))
x11()
plot(mod.mac, disp = c("sites", "cn"))
...
colSums(fmc > 0)
   ...
   strat.plot(100 * (ffosmc / rowSums(ffosmc))[order(cfm$O2), colSums(ffosmc > 0) > 10],  cfm$O2[order(cfm$O2)], wa.order = "topleft", scale.percent = TRUE)   
strat.plot(100 * (ffosmc / rowSums(ffosmc))[order(cfm$TOC), colSums(ffosmc > 0) > 10], cfm$TOC[order(cfm$TOC)], wa.order = "topleft", scale.percent = TRUE)   

x11()
strat.plot(100 * (fall / rowSums(fall))[order(chem$O2), colSums(fall > 0) > 10],  chem$O2[order(chem$O2)], wa.order = "topleft", scale.percent = TRUE, ylabel = expression(O[2]), plot.line = FALSE, lwd.bar = 2, col.bar = 1, cex.xlabel = .7)   
x11()
strat.plot(100 * (fmc / rowSums(fmc))[order(cfm$TOC), colSums(fmc > 0) > 10], cfm$TOC[order(cfm$TOC)], wa.order = "topleft", scale.percent = TRUE, ylabel = "TOC", plot.line = F, lwd.bar = 2, col.bar = 1, cex.xlabel = .7, y.rev = TRUE)   
x11()
strat.plot(100 * (fall / rowSums(fall))[order(div(fall)), colSums(fall > 0) > 10], exp(div(fall))[order(div(fall))], wa.order = "topleft", scale.percent = TRUE, ylabel = "exp(H')", plot.line = FALSE, lwd.bar = 2, col.bar = 1, cex.xlabel = .7)   

x11()
strat.plot(100 * (macro8g / rowSums(macro8g))[order(div(macro8g)), colSums(macro8g > 0) > 10], exp(div(macro8g))[order(div(macro8g))], wa.order="topleft", scale.percent = TRUE, ylabel = "exp(H')", plot.line = FALSE, lwd.bar = 2, col.bar = 1, cex.xlabel = .7)   

wa <- WA(sqrt(fall / rowSums(fall))[!is.na(chem$mO2), colSums(fall > 0) > 4], chem$mO2[!is.na(chem$mO2)], tolDW = TRUE)
opt <- wa$coef
opt <- opt[order(opt[, 1]), ]
x11()
par(mar = c(3, 8.5, 1, .5), mgp = c(1.5, .5, 0),  tcl = -.4)
...
wa <- WA(sqrt(macro8g / rowSums(macro8g))[rowSums(macro8g > 0) > 4, colSums(macro8g > 0) > 5],  exp(div(macro8g))[rowSums(macro8g > 0) > 4], tolDW = TRUE)
opt <- wa$coef
opt <- opt[order(opt[, 1]), ]
x11()
par(mar = c(3, 8.5, 1, .5), mgp = c(1.5, .5, 0), tcl = -.4)
