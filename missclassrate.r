...
fallrep<-   xtabs(Number.1~paste(Station_code, Replicate, sep=":")+SpeciesForam, data=forams, drop=T)
...
x11();par(mfrow=c(3,1), mar=c(0,3,.5,1), oma=c(3,0,1,0), mgp=c(1.5,.5,0))
...
abline(v=boundaries)
axis(1, outer=T)
title(xlab="Exponential Shannon", outer=T)
